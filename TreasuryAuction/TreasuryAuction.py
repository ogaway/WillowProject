...
def session(me):
    ...
    if me == 0:
        ...
        for time in range(Round):
            ...
            if auction == 1:
                ...
                for i in range(goodsnum):
                    ...
                    add("被験者No.%sは%s円で国債を１つ落札しました。<br />" % (info[i]["client"], price))
                for i in range(goodsnum, len(info)):
                    put({"tag": "e", "client": info[i]["client"], "get": 1})
                    add("被験者No.%sは国債を１つ落札できませんでした。<br />" % info[i]["client"])
            # Spanish Designs
            ...
            add("平均勝ち入札価格は%s円、平均落札価格は%s円でした。<br />" % (averagebid, averageprice))
            ...
            if time != Round-1:
                ...
                "<input id='go' type='submit'></p>" % (time+2))
                take({"client": me})
                starthide(time+2)
                # Put Result(h)
                wiput(plnum, {"tag": "h", "result": result})
                for i in range(plnum):
                    add("%s位：被験者No.%s, %s円<br />" % (i+1, result[i]["client"], result[i]["profit"]))
        add("<p>これで実験を終了します。</p>")
        ...
        if auction == 0:
            add(open("TAsubject_DESC.html"))
        # Uniform Designs Instruction
        elif auction == 1:
            add(open("TAsubject_UNI.html"))
        # Spanish Designs Instruction
        elif auction == 2:
            add(open("TAsubject_SPAN.html"))
        add(plnum, "#plnum")
        ...
        for time in range(Round):
            ...
            if time == 0:
                ...
                hide("#instruction")
                add("<h2>情報板</h2>")
                add("<p>現在の合計利潤：<span id='profit'>0</span>円</p>")
                add("<p id='info'></p>")
            elif time >= 1:
                waitinfohide(2*time)
                boardhide(time-1)
